...
export default function Card({
  ...
  return (
    <div className="relative flex flex-col text-gray-700 bg-transparent w-full">
      <Link
        href={slug}
        className="relative mx-4 mt-4 overflow-hidden text-gray-700 bg-transparent rounded-xl h-52"
      >
        <picture>
          <img
            src={image}
            alt={title}
            className="bg-cover bg-center transition-all duration-300 ease-in-out group-hover:scale-105"
          />
        </picture>
      </Link>
      <div className="p-6 text-start">
        <h4 className="block mb-2 font-sans text-xl antialiased font-semibold leading-snug tracking-normal text-gray-800">
          {title}
        </h4>
      </div>
      <div className="flex justify-start p-6 pt-2 gap-4 ">
        <a
          href={github_url}
          target="_blank"
          className="block font-sans text-lg antialiased font-normal leading-relaxed text-gray-600 hover:text-gray-900 p-2 rounded-md cursor-pointer"
        >
